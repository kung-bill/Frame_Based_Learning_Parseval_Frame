...
for iterNum = 1:param.numIteration
    ...
    if (param.errorFlag==0)
        ...
        CoefMatrix = OMP([FixedDictionaryElement,Dictionary],Data, param.L, param.errorGoal);
    else 
    ...
    if (iterNum>1 & param.displayProgress)
        if (param.errorFlag==0)
            ...
            Error_matrix = (Data-[FixedDictionaryElement,Dictionary]*CoefMatrix);
            disp(['Iteration   ',num2str(iterNum),'   max error norm is: ',num2str(max(sqrt(sum(Error_matrix.*Error_matrix))))]);
        else
...
function [A]=OMPerr(D,X,error_norm); 
%=============================================
...
function [A]=OMP(D, X, L, error_norm); 
%=============================================
% Sparse coding of a group of signals based on a given 
% dictionary and specified number of atoms to use. 
% input arguments: 
...
for k=1:1:P,
    ...
    for j=1:1:L,
        ...
        a=pinv(D(:,indx(1:j)))*x;
        residual=x-D(:,indx(1:j))*a;
        if sum(residual.^2) < error_norm^2
            break;
        end
    end;
    temp=zeros(K,1);
